...
class Time {
    ...
    private function welcomeMessage($currentHour, $weatherTip, $weatherTipTomorrow) {
      ...
      //check that the weather Tips are valid
      if($weatherTip ===false || $weatherTipTomorrow ===false) {
    ...
    public function welcomeWidget($currentWeather, $tomorrowsWeather) {
      ...
      if ($currentWeather === false || $tomorrowsWeather === false) {
          $buffer = "Hi Mike";
      }
      else {
    ...
    //fetch the correct wallpaper override based on the time of day
    public function wallpaperCss() {
        ...
        if ($currentHour < $afternoonCutoff) {
          ...
          $wallpaper = "morningWallpaper.css";
        }
        else if ($currentHour < $eveningCutoff) {
          
          $wallpaper = "afternoonWallpaper.css";

        }
        else if ($currentHour >= $eveningCutoff) {
          
          $wallpaper = "nightWallpaper.css";
        }
      
     
        //ensure that we only ever return an empty string if nothing is preset.
        if(is_null($wallpaper) || $wallpaper == "") {
          $wallpaper = "";
        }
        else {
          $wallpaper = "<link rel='stylesheet' type='text/css' href='./css/" . $wallpaper. "'>";
